...
SHEET_HEADERS = [
    "Ticket ID", "Category", "Customer Name", "Customer Email",
    "Order Number", "Priority", "Status", "Issue Summary", "Timestamp", "Notes",
]

_worksheet_cache = None   # cached after first successful connection
_headers_written = False  # only verify headers once per process lifetime


def _get_sheet():
    """Return the cached worksheet, connecting once per process."""
    global _worksheet_cache, _headers_written
    import gspread
    ...
    if _worksheet_cache is None:
        ...
        _worksheet_cache = gc.open_by_key(sheet_id).sheet1

    # Verify headers only once per process restart
    if not _headers_written:
        existing = _worksheet_cache.row_values(1)
        if existing != SHEET_HEADERS:
            _worksheet_cache.delete_rows(1)
            _worksheet_cache.insert_row(SHEET_HEADERS, 1)
